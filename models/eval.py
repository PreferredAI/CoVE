...
from cornac.models import NextItemRecommender
...
def ranking_eval(
    ...
    for [user_idx], [sid], [mapped_ids], [session_items] in tqdm(
        ...
        history_items = session_items[:-1] + [session_items[-1][:-1]] if mode == "last" else session_items[:-1]
...
class NextSessionEvaluation(BaseMethod):
    ...
    def evaluate(self, model, metrics, user_based, show_validation=True):
        ...
        if len(rating_metrics) > 0:
            warnings.warn(
                "NextItemEvaluation only supports ranking metrics. The given rating metrics {} will be ignored!".format(
                    [mt.name for mt in rating_metrics]
                )
            )
