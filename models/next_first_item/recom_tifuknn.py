...
class TIFUKNN(NextItemRecommender):
    ...
    def fit(self, train_set, val_set=None):
        ...
        if self.n_neighbors > len(self.user_vectors):
            warnings.warn(
                "Number of users is %d, smaller than number of neighbors %d"
                % (len(self.user_vectors), self.n_neighbors)
            )
    ...
    def _get_user_vectors(self, data_set):
        user_vectors = []
        for _, _, _, [batch_items] in tqdm(
            data_set.usi_iter(batch_size=1, shuffle=False),
            desc="Getting user vectors",
            total=data_set.num_users,
        ):
            user_vectors.append(self._compute_user_vector(batch_items[:-1]))
    ...
    def score(self, user_idx, history_items, **kwargs):
        ...
        user_vector = self._compute_user_vector(history_items)
