...
# def moe_io_iter(usi_iter, uir_tuple, n_sample=0, sample_alpha=0, rng=None, batch_size=1, shuffle=False):
...
#     # for _, batch_mapped_ids in usi_iter(batch_size, shuffle):
...
#                 yield input_uids, input_iids, output_iids, start_mask, np.arange(batch_size, dtype="int")
...
#             negative_samples = rng.choice(item_indices, size=n_sample, replace=True, p=item_dist)
#             output_iids = np.concatenate([output_iids, negative_samples])
#         yield input_uids, input_iids, output_iids, start_mask, np.nonzero(valid_id)[0]
